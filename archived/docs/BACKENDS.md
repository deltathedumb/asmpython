...
## Options your backend takes from the command line
...
class MyBackend(Backend):
    name = "my-machine"
    options = (
        Option("isa-level", "which revision of the ISA to emit for",
               metavar="LEVEL"),
    )

    def __init__(self, isa_level: int = 1):
        self.isa_level = isa_level

    def configure(self, values, sink) -> Backend:
        try:
            level = int(values.get("isa-level", 1))
        except ValueError:
            raise OptionError("--isa-level wants a number") from None
        return MyBackend(level)
```
...
without the dashes, and returns the backend to emit with. Raise `OptionError`
for a value nobody can act on and it becomes a diagnostic; report anything
advisory to `sink` and it becomes a warning.

**Return a new instance. Do not mutate `self`.** The registry holds one shared
backend object, so a backend that stored its flags on itself would leak them
into the next compilation in the same process — invisible in a command-line
run, and wrong in every test suite and embedding tool.

Passing a flag to a backend that does not declare it is an error naming the
backend that does, so `--isa-level` with `--backend c` fails rather than being
quietly ignored. The `jvm` backend is the worked example: `--java-version` and
`--class-version` both decide its class-file version, and
`backends/jvm/version.py` is the whole of that decision.

## Modules your backend can offer

A backend can make things importable. A board gets `import hw`; a hosted
backend gets whatever its host has.
...
class MyBackend(Backend):
    ...
    modules = {
        ...
        "json": {"parse": ("call", "hw_json_parse", 1)},
    }
```

Each member is `(kind, payload)`, the same table
`frontends/python/modules.py` documents: `("int", n)`, `("float", x)`,
`("str", s)` for a constant, and `("call", symbol, arity)` for a runtime
function, which becomes a callable value so it can be passed as well as
called.

**Names resolve by three rules, in order:**

| written | reaches |
|---|---|
| `import my-machine.json` | **always** your module, collision or not |
| `import math` | the standard module — it wins the bare name |
| `import hw` | yours, because nothing else wanted `hw` |

So pick any name you like. If it collides, the pre-existing module keeps the
bare name and yours is reached through the prefix — which is where the
ambiguity was in the first place, and the prefixed path works whether or not
there is a collision, so a program that wants to be explicit can always be.

A pre-existing module winning is deliberate: a program that said `import json`
before your backend grew one of its own must keep meaning what it meant.

`import a.b` binds `a`, a package holding `b`, as Python does; `import a.b as
n` binds `n` to the module. Two spellings of one module are one object, so
`import hw` and `import my-machine.hw` give `hw is my_machine_hw`.

Which backend is compiling decides what is importable, so this is published
before the frontend runs. A program using your modules does not compile under
another backend, which is the honest outcome — the diagnostic lists what that
backend does offer.
