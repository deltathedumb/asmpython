...
Target(name, arch, os, abi, object_format,
       ...
       object_suffix, executable_suffix, cc_names, default_toolchain)
```
...
## A target whose artifacts are not objects names its toolchain

`cc_names` answers *which* C driver. `default_toolchain` answers the prior
question — whether a C driver is involved at all.

```python
register(Target(
    "jvm", arch="jvm", os="jvm", abi="jvm",
    object_format="class", object_suffix=".class", executable_suffix=".jar",
    default_toolchain="jar",
), aliases=("java",))
```

Nothing `gcc` can do with a class file, so the `jvm` target names the toolchain
that packages one. Empty — the default — means the C compiler driver, which is
right for every target that emits objects or assembly.

This is a field for the same reason `abi` is one. The link stage used to pick
the bare-metal toolchain by testing `target.os == "none"`, which is a guess
that happens to be right for the targets that shipped; a third-party target
whose artifacts are neither objects nor source had no way to say so, and would
have been handed to `gcc`.
