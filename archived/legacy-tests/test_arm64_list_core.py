...
from asmpython._backends.arm64._verify_list_core import (
...
class Arm64ListCoreTests(unittest.TestCase):
    def test_probe_lowers_to_exact_non_exception_runtime_surface(self) -> None:
        blob = compile_source_object(_LIST_SOURCE)
    ...
    def test_list_runtime_is_accepted_before_tool_discovery(self) -> None:
        blob = compile_source_object(_LIST_SOURCE)
        self.assertEqual(
    ...
    def test_manifest_exports_first_list_slice(self) -> None:
        self.assertIn("_abi_new_list", RUNTIME_EXPORTS)
        self.assertIn("_abi_list_append", RUNTIME_EXPORTS)

    def test_growth_contract_matches_reference_backend(self) -> None:
        cap = 2
        length = 2
        if length >= cap:
            cap = max(cap * 2, 4)
