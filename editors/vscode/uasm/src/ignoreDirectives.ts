...
// `# uasm: ignoreall` anywhere in the file suppresses every diagnostic
// for that document. `# uasm: ignore` at the end of (or alone on) a
...
// `# type: ignore`.
const IGNORE_ALL_RE = /#\s*uasm:\s*ignoreall\b/i;
const IGNORE_LINE_RE = /#\s*uasm:\s*ignore\b(?!all)/i;
