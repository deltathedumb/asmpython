...
@harness.fixture
def tree(tmp_path):
    """A miniature repository: `src/uasm/thing.py` and nothing else."""
    pkg = tmp_path / "src" / "uasm"
...
def test_a_snapshot_copies_the_tree(tree):
    ...
    assert (taken / "uasm" / "thing.py").read_text(encoding="utf-8") \
        == "VALUE = 'original'\n"
...
def test_editing_the_original_does_not_change_the_snapshot(tree):
    ...
    (tree / "src" / "uasm" / "thing.py").write_text(
...
def test_stale_bytecode_is_left_behind(tree):
    ...
    assert not (taken / "uasm" / "__pycache__").exists()
    snapshot.discard(taken)
...
def test_discard_refuses_to_delete_a_real_source_tree(tree):
    ...
    assert (tree / "src" / "uasm" / "thing.py").exists()
