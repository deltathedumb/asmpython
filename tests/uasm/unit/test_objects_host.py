...
interpreter traps, and nothing notices until someone runs `uasm run` on a
...
`uasm run` trapped on all of them -- 118 of 177 symbols were unbound
...
from uasm.objects.host import _TABLE
from uasm.objects.csource import OBJECT_NAMES, OBJECTS_C
...
#: defines is reachable from the interpreter, so `uasm run` and a compiled
...
def test_every_runtime_symbol_has_a_host_binding():
    ...
    assert not added, (
        ...
        + "\n  ".join(added)
        + "\n\nA compiled program can call these and `uasm run` cannot. "
...
def test_every_exported_symbol_has_a_host_binding():
    ...
    program through `uasm run` found it.
