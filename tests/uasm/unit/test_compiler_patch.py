...
A registry covers the four extension points uasm anticipated. A patch
...
from uasm.plugins import CompilerPatch, PatchError
from uasm.plugins import patch as patch_module
...
class TestWhatItRefuses:
    @harness.cases("target", [
        "uasm.plugins.patch.apply_all",
        "uasm.plugins.patch.CompilerPatch.check",
        "uasm.plugins.store.write",
        "uasm.plugins.store.remove",
    ])
    ...
    @harness.cases("target", [
        "uasm.ir.verifier.verify",
        "uasm.backend.base.register",
        "uasm.target.registry.register",
    ])
    ...
    def test_force_allows_a_guarded_target(self):
        p = CompilerPatch("uasm.ir.verifier.verify",
    ...
    def test_everything_else_is_open(self):
        ...
        p = CompilerPatch("uasm.passes.transforms.DeadCodeElimination.run",
...
class TestBadTargets:
    def test_a_module_is_not_a_target(self):
        with harness.raises(PatchError, match="names a module"):
            CompilerPatch("uasm.ir.printer", replace=print).apply()

    def test_a_missing_attribute_is_reported(self):
        with harness.raises(PatchError, match="does not exist"):
            CompilerPatch("uasm.ir.printer.nope", replace=print).apply()
    ...
    def test_a_non_callable_is_refused(self):
        with harness.raises(PatchError, match="not callable"):
            CompilerPatch("uasm.plugins.MANIFEST_ATTR",
                          replace=print).apply()

    def test_a_method_can_be_patched(self):
        """`pkg.mod.Class.method` needs no special spelling."""
        p = CompilerPatch("uasm.ir.module.Module.statistics",
...
class TestConflicts:
    ...
    def test_a_failing_batch_leaves_nothing_applied(self):
        ...
        bad = CompilerPatch("uasm.plugins.store.write", replace=print)
