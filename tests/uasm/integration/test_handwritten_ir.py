...
from uasm.backend import get as get_backend, load_builtin
...
def compile_and_run(module, backend_name: str, target_name: str, tmp_path):
    ...
    for name, data in artifacts.items():
        ...
        inputs.append(str(p))
    if backend_name != "c":
        from uasm.objects import write_runtime
...
class TestTheNameManglingItself:
    def test_c_keywords_are_renamed(self):
        from uasm.backends.c.emit import _cname
    ...
    def test_labels_cannot_collide_with_a_keyword(self):
        from uasm.backends.c.emit import _label
