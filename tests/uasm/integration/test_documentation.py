...
are treated differently:
  ...
  * LANGUAGE examples in LANGUAGE.md are uasm source, not driver code,
...
def _readers_objects() -> dict:
    ...
    """
    import tempfile
    from uasm.link.base import LinkRequest

    work = Path(tempfile.mkdtemp(prefix="uasm-docs-"))
    source = work / "prog.py"
    source.write_text(
        "def main() -> int:\n"
        "    total: int = 0\n"
...
def _sessions() -> dict[str, list[tuple[int, str, bool]]]:
    ...
    for doc in DOCS:
        if doc.name == "LANGUAGE.md":
            continue                      # uasm source, compiled below
        defined = set(_READER)
        blocks = []
...
def test_the_version_is_one_number() -> None:
    """`VERSION`, the build and `uasm.__version__` say the same thing.
    ...
    assert uasm.__version__ == declared, (
        f"uasm.__version__ is {uasm.__version__!r}, "
...
def test_the_readme_layout_lists_the_real_packages() -> None:
    ...
    actual = {p.name for p in (ROOT / "src/uasm").iterdir()
...
def test_every_language_example_compiles(line: int, source: str,
                                         tmp_path: Path) -> None:
    """LANGUAGE.md says what the subset accepts. uasm decides.
...
def test_the_frontend_is_not_a_subset(what: str, source: str,
                                      tmp_path: Path) -> None:
    """uasm compiles Python, and a claim otherwise must not survive.
    ...
    asking what uasm accepts found a sentence four years out of date and
