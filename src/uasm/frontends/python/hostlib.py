"""The host Python installation's packages, as a LIBRARY POINT.
    ...
    uasm build prog.py          # `import requests` now resolves
...
user just ran, in the overwhelmingly common case where uasm was installed
...
def _probe_self() -> dict | str:
    ...
    Not merely an optimisation: uasm may be running from a frozen or
...
def _probe_subprocess(python: str) -> dict | str:
    try:
            ...
            # uasm. `-I` already ignores it; removing it as well covers
