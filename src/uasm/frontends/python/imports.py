...
uasm compiles a file. Real programs are directories, and until now
...
class Finder:
    ...
    def absolute(self, name: str, level: int, package: str) -> str:
        ...
        `from ...ir import types` inside `uasm.frontends.python` is level
        3, which strips three components off `uasm.frontends.python` --
    ...
    def find_before_stdlib(self, name: str) -> Path | None:
        ...
        program printed the bundled answer under uasm and the file's own
    ...
    def package_of(self, path: Path) -> str:
        ...
        # `src/uasm/ir/types.py` is `uasm.ir.types` under `src` and
        # a bare `types` under its own directory, which is always a root.
        #
        # Taking the first match made every relative import in uasm's own
