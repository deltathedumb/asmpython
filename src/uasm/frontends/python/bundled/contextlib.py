...
COVERAGE: `contextmanager`, `suppress`, `ExitStack` (`callback`, `push`,
`enter_context`, `pop_all`, `close`), `nullcontext`.

NOT COVERED: `asynccontextmanager`, `AsyncExitStack`, `aclosing`,
`closing`, `redirect_stdout`, `redirect_stderr`, `chdir`, `AbstractContextManager`
as a base to inherit from, and reusing a `contextmanager` result twice --
CPython raises `RuntimeError: generator didn't stop` and this does not check.
...
HELD BACK ONCE, AND WHY IT IS BACK. When the standard library was cleared this
module was rebuilt and then withheld: its `__exit__` called `gen.throw`
correctly and the cleanup still did not run, while the same generator throwing
into the same `finally` worked in isolation. A context manager whose cleanup
silently does not run is worse than an absent one, so it stayed out until the
fault was understood rather than until it happened to pass.

It was never a fault in this module. Taking the shape apart one layer at a
time -- a generator resumed past its last yield, then from a function, then
from a method, then through the `with` protocol, then through a factory --
every layer already agreed with CPython, and so did the module once the
compiler fixes of the same session landed. See `docs/STDLIB.md`.
"""
