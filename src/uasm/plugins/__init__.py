...
then:
    ...
    uasm plugin add my_plugin_module      # remembered from now on
    uasm plugin list
...
uasm was embedded as a library.

Four ways a module gets imported, in this order:

    1. `uasm plugin add NAME`      -- persisted; loaded on every run
    2. `--plugin NAME`                  -- this invocation only
    3. `UASM_PLUGINS=a:b`          -- for a CI job or a Makefile
    4. an `uasm.plugins` entry point of an installed distribution

A module may either declare `__uasm_plugin__` or call `register()` at
import time; both work. Declaring is better, because `uasm plugin show`
...
def load_all(
    ...
    including `uasm plugin remove`, the one command that would fix it.
    """
    report = LoadReport()
...
def install(
    ...
    *, replace: bool = False,
    ...
    """Resolve, load, and remember. What `uasm plugin add` does.
    ...
    of uasm -- including `plugin remove` -- would fail on loading it.
    """
    ...
    try:
        ...
        # user's own file -- which reads as "you found a bug in uasm".
        raise PluginError(name, exc) from exc
