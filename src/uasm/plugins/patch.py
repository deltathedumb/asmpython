...
A registry covers the four things uasm expected people to extend. This
...
worth rewording, an experiment you would otherwise maintain as a fork:
    ...
    from uasm.plugins import CompilerPatch

    def louder(original, module, sink):
        sink.report(...)
        return original(module, sink)

    patch = CompilerPatch("uasm.passes.dce.run", wrap=louder,
...
WHAT CANNOT BE PATCHED, and why there are two tiers:
  ...
  * GUARDED -- the verifier and the four registries. Patchable, but only by
    saying `force=True`, which `uasm plugin show` then displays. These
...
it is how names work in Python -- so patch what the callers actually reach:
    ...
    uasm.passes.transforms.dce            # patched: callers use the
                                               # module attribute
    uasm.driver.pipeline.compile_source   # patched, but cli.py did
...
class CompilerPatch:
    ...
    `target` is a dotted path: `uasm.passes.dce.run`, or
    `uasm.ir.builder.Builder.emit` for a method. The module part is
