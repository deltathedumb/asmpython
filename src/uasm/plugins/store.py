...
`uasm plugin add mypack` has to outlive the process or it is not an
...
The location follows the platform, and `UASM_CONFIG_DIR` overrides it.
...
ENV_CONFIG_DIR = "UASM_CONFIG_DIR"
...
def read() -> list[Entry]:
    ...
    are enough to work, and a user who cannot run `uasm plugin remove`
