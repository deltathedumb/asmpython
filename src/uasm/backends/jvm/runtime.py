...
(`uasm.objects.support`). There is nothing to link here -- a class file is
...
in `uasm.objects.support`, because a program's output must not depend on
...
def _load_bytes(rt: Runtime, m: MethodBuilder, count: int, *,
    ...
    that disagree with `uasm run` only for code that stores one width and
    loads another.
    """
    m.load("l", 0)
    m.op("l2i")
...
def _repr(rt: Runtime, m: MethodBuilder) -> None:
    ...
    A transcription of `py_repr_double` in `uasm.objects.support`, and it has
...
# than under `uasm run` or the C backend.
#
# A transcription of `POW_INT_C` in `uasm.objects.support`, down to the
