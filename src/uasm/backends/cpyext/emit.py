...
WHAT THIS IS NOT. `objects/c/` is uasm's own dynamic-object runtime --
...
inside uasm's own arena -- never touching a real `PyObject` -- and
...
called from other uasm code, and converts the single returned
...
/* THE NAMES `apy_error_type()` HANDS BACK are the uasm runtime's own
...
/* THE UASM RUNTIME'S ERROR STATE IS STICKY AND GLOBAL (`apy_err_type`
