"""Linking: backend artifacts to a program you can run.
    ...
    from uasm.link import Toolchain, register
...
Then `uasm build prog.py --toolchain bare-metal`. See `docs/LINKERS.md`.

THE OBJECT RUNTIME IS NOT HERE. It was, and it was most of this package by
line count, which made `from uasm.link import ...` the way a frontend
asked what `apy_add` takes. It lives in `uasm.objects` now, and what is
