...
`verify()`, all of this holds:
  ...
  3.  Every register read is declared in `func.registers`, and READ AFTER A WRITE on
...
from .module import Block, Function, Instruction, Module
from .opcodes import Op, Sig
...
def verify(module: Module) -> None:
    ...
    problems: list[str] = []
    known = {f.name for f in module.functions}
...
def _func(fn: Function, known: set[str], globs: set[str], out: list[str]) -> None:
    where = fn.name
...
def _block(fn: Function, blk: Block, labels: set[str], known: set[str],
    ...
    for i, ins in enumerate(blk.instructions):
        ...
        if ins.is_terminator and not last:
            out.append(
                ...
                f"{len(blk.instructions) - i - 1} more instruction(s)"
            )
...
def _instr(fn: Function, where: str, ins: Instruction, labels: set[str],
    ...
    if spec.defines_value:
        ...
        elif ins.dst in fn.registers and fn.registers[ins.dst] != want:
            out.append(
                f"{where}: {name} defines %{ins.dst} as {fn.registers[ins.dst]} but "
                f"produces {want}"
            )
...
def _check_reads_after_writes(fn: Function, out: list[str]) -> None:
