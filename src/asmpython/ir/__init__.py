"""UIR -- a Universal Intermediate Representation.

THE NAME DOESN'T MENTION THE COMPILER, and that is the point rather than an
oversight. This IR is small, typed and language-independent; the Python
frontend is its first client and is not meant to be its only one, so naming it
after the compiler it ships in (`uasm`) would have made a claim to
universality and then quietly undercut it. The letters are the container's
magic bytes too -- see `backends/uir/emit.py`, where they were already written
down.
...
of them importing `uasm.frontends.python.methods`, the bottom layer of the
compiler reaching for the top. Those live under `objects/` now, beside the C
and subset arrangements of the same runtime, and
`tests/uasm/unit/test_ir_is_agnostic.py` is what keeps them out: the IR
...
docs. A backend author should need no second document.
    ...
    from uasm.ir import Module, Function, Builder, verify
