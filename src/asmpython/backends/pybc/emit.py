...
THE ESCAPE HATCH IS THAT ONE ALREADY SHIPS WITH EVERY COPY OF ASMPYTHON.
`pyproject.toml` requires Python 3.14 to even RUN asmpython, and for the
...
source as written, not the asmpython IR after optimisation passes have run
over it -- `-O` (asmpython's pass pipeline) has no effect on this backend's
...
source to be within whatever asmpython's frontend accepts, same as every
other backend, even though the bytes this backend actually emits never
touch the IR that pipeline produced.

`asmpython build prog.py --backend pybc` produces a single `.pyc`, in the
...
See `tests/asmpython/unit/test_pybc_backend.py`.
"""
...
class PycBackend(Backend):
    ...
    def emit(self, module: Module, target: Target) -> dict[str, bytes]:
        ...
        try:
            ...
            # asmpython accepting something narrower than full CPython (a
