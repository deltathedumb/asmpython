...
class Arm64Backend(Backend):
    ...
    def emit(self, module: Module, target: Target) -> dict[str, bytes]:
        ...
        for fn in module.defined_functions():
            out.extend(self._function(fn, abi))
            out.append("")
    ...
    # ── one function ────────────────────────────────────────────────────────
    def _function(self, fn: Function, abi: ABI) -> list[str]:
        ...
        name = self.symbol(fn.name)
        if fn.linkage is Linkage.EXPORT:
            e.lines.append(f"\t.globl {name}")
        e.lines.append(f"\t.type {name}, %function")
        e.label(name)
        ...
        e.lines.append(f"\t.size {name}, .-{name}")
        return e.lines
    ...
    def _instruction(self, e: _Emitter, ins: Instruction, saved: list[str],
                     saved_base: int, frame: int, abi: ABI) -> None:
        op, ty = ins.op, ins.ty
        fn_name = e.fn.name
        ...
        match op:
            ...
            case Op.GLOBAL_ADDR | Op.FUNC_ADDR:
                dest = e.out_register(ins.dst, SCRATCH_A)
                target = (self.symbol(ins.sym) if op is Op.FUNC_ADDR
                          else ins.sym)
            ...
            case Op.CALL | Op.CALL_PTR:
                ...
                if op is Op.CALL:
                    e.emit(f"bl {self.symbol(ins.sym)}")
                else:
