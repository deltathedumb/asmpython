...
WHAT THIS DEMONSTRATES that the C backend cannot: using `asmpython.backend.regalloc`.
...
class UnsupportedOperation(BackendUnsupported):
    ...
    compiler stack in it, which says "you found a bug in asmpython" when the true
