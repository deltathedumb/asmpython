...
COVERAGE: ModuleType, SimpleNamespace, new_class.

NOT COVERED: every other name -- FunctionType, MethodType, GeneratorType, MappingProxyType, the coroutine and frame types, and
`prepare_class`. Most are objects this runtime has no separate type for.

Restored from `archived/stdlib-prerefactor/` and measured against CPython by
`tests/stdlib/types.py`. The coverage line above is the module's contract; see
`docs/STDLIB.md`.
