...
COVERAGE: complete. Every feature name, each as a `_Feature` with its real
`optional` and `mandatory` release tuples and its `compiler_flag`,
`all_feature_names`, and the `CO_*` flag constants.

LOST AND RECOVERED. This module was deleted by the standard-library clear and
was NOT copied into `archived/stdlib-prerefactor/` with the other twenty-seven
-- so unlike them it had no reference version to rebuild from, and the only
copy was in git. The conformance case `pep/0236-future-statement` is what
noticed, by failing with `'int' object has no attribute 'optional'`.

Restored from `f7d39766^` rather than rewritten, because it was complete and
correct; what it was missing was a coverage line and a test, which it now has.

READ IT BY NAME AND NOT THROUGH `getattr`. There are two `__future__`s: this
one, spliced in and reached by writing `__future__.annotations` out; and the
compiler's own module table (`frontends/python/modules.py`), where the same
names are the integer compiler flags. `import __future__` keeps binding the
second -- the statement SURVIVES the splice precisely because something else
also provides the name -- so `__future__.annotations` is a `_Feature` and
`getattr(__future__, "annotations")` is an `int`. Writing the attribute out is
what a program does and what is tested; the dynamic spelling reaches the other
table and is not something this module can answer for.
