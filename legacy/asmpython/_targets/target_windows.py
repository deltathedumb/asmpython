...
from .._compiler.codegen import Codegen, FuncInfo
...
class WindowsCodegen(Codegen):
    ...
    def _emit_gc_globals_fallback(self) -> None:
        ...
        HISTORY, because this was disabled once for the wrong reason: the range
        this computes is CORRECT and always was -- it reproduces the static
        section union exactly, and the module globals do live inside it. What
        was broken is that they sat at UNALIGNED addresses (measured at
        0x140007247, i.e. 7 mod 8) because the linker started the mutable-data
        bucket wherever the rodata blob happened to end.
        `_runtime_gc_scan_range` walks eight bytes at a time from a
        page-aligned base, so it visited only addresses congruent to 0 mod 8
        and stepped over every global slot -- finding no roots and letting the
        sweep free live module-level data. The fix is in pe_linker (align the
        data bucket), not here.
