"""Verify exact ARM64 str.replace runtime behavior."""
...
_REPLACE_SOURCE = """\
def main() -> int:
    print("banana".replace("ana", "X"))
    print("aaaa".replace("aa", "b"))
    print("abc".replace("", "-"))
    print("éé".replace("é", "λ"))
    print("café".replace("é", "e"))
    print("unchanged".replace("x", "y"))
    print("ab".replace("b", "LONG"))
    return 0
"""
_EXPECTED_STDOUT = "bXna\nbb\n-a-b-c-\nλλ\ncafe\nunchanged\naLONG\n"
_EXPECTED_REQUIREMENTS = frozenset({"_abi_str_replace", "printf"})
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="asmpython-arm64-replace-") as tmp:
        executable = Path(tmp) / "program"
        program_blob = compile_source_object(_REPLACE_SOURCE)
        requirements = required_external_symbols(program_blob)
        if requirements != _EXPECTED_REQUIREMENTS:
            raise SystemExit(
                "string-replace probe lowered to an unexpected external-symbol set: "
        ...
        if completed.returncode != 0 or completed.stdout != _EXPECTED_STDOUT:
            ...
            raise SystemExit(
                "freestanding ARM64 string-replace probe failed: "
                f"returncode={completed.returncode}"
            )

        mode_name = "native AArch64" if toolchain.native else "qemu-aarch64"
        print("[ OK ] non-overlapping replacement matched Python")
        print("[ OK ] empty-old replacement inserted at Unicode code-point gaps")
        print("[ OK ] shrinking, growing, UTF-8, and no-match cases succeeded")
