"""Verify exact ARM64 math.isinf classification."""
...
def main() -> int:
    print(isinf(inf), isinf(-inf), isinf(nan), isinf(1.0))
    return 0
"""
_EXPECTED_STDOUT = "1 1 0 0\n"
_EXPECTED_REQUIREMENTS = frozenset({"_abi_int_to_base", "_math_isinf", "printf"})
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="asmpython-arm64-isinf-") as tmp:
        executable = Path(tmp) / "program"
        program_blob = compile_source_object(_ISINF_SOURCE)
        requirements = required_external_symbols(program_blob)
        if requirements != _EXPECTED_REQUIREMENTS:
            raise SystemExit(f"isinf probe external symbols: expected={sorted(_EXPECTED_REQUIREMENTS)}, actual={sorted(requirements)}")
        ...
        if completed.returncode != 0 or completed.stdout != _EXPECTED_STDOUT:
            ...
            raise SystemExit(f"freestanding ARM64 isinf probe failed: returncode={completed.returncode}")
        mode_name = "native AArch64" if toolchain.native else "qemu-aarch64"
        print("[ OK ] only positive and negative infinity matched")
