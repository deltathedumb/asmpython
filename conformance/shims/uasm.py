"""Shim for uasm: compile the case to a native binary, then run it.

The subject is the compiler in `src/uasm` -- the 3.14 tree. It is put on
the path explicitly rather than left to `import uasm`, because a stale
...
_UASM_ROOT = Path(__file__).resolve().parents[2]
...
# has a released `uasm` in site-packages answering to the same import name,
# and `-m uasm` finds THAT unless the checkout is placed ahead of it. The
...
# The compiler builds its runtime into a SHARED directory (uasm/_runtime/
...
def run(case_path: str, timeout: int):
    ...
    # Each compile gets its OWN workdir. The default is `.uasm` under the
