...
_NON_BUILD_COMMANDS = MANAGEMENT_COMMANDS | {
    "extension", "package", "pypi", "pyinbin", "project", "sign", "abi", "verify", "lock",
}
...
def _extensions_needed(raw: list[str], *, is_build: bool) -> bool:
    if is_build:
        return True
    return bool(raw and raw[0] in {"backends", "linkers", "ir", "test", "lock"})
...
def _legacy_target_argv(argv: list[str]) -> list[str]:
    """Lower a normalized triple to the OS value understood by the old parser."""
    result = list(argv)
    for index, token in enumerate(result[:-1]):
        ...
        if len(parts) == 3:
            ...
            system = parts[1]
            result[index + 1] = {"macos": "mac"}.get(system, system)
    return result


def _postprocess_artifact(raw: list[str], options: SharedBuildOptions, report) -> int:
...
def _main_with_options(
    ...
    *,
    ...
    if result != 0:
        return result
    return _postprocess_artifact(raw, options, report)
