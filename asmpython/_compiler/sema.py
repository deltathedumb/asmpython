...
class SemaAnalyzer:
    ...
    def _transform_generator(self, f: A.FuncDef) -> "tuple[A.FuncDef, A.ClassDef] | None":
        ...
        One encoding, no shape matching: `_transform_generator_cfg` flattens
        the body into basic blocks and dispatches on a state field, so a resume
        point is a state number rather than a recognised syntactic form.

        Returns (factory_func, iterator_class), or None for the constructs the
        flattener declines -- a `yield` inside try/with/match, or inside a
        tuple-target `for`. The two shape-matching transforms that used to
        stand behind this declined those same shapes, so nothing that reached
        them still does.
        """
        return self._transform_generator_cfg(f, f"_genobj_{f.name}", f.pos)
