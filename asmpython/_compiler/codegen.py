...
# Runtime type/exception IDs now live in type_ids.py -- they are a runtime
# CONTRACT shared by ir_lower, sema and every backend, not something this
# backend owns. Re-exported here so existing importers keep working.
from .type_ids import (  # noqa: F401
    EXC_ANY,
    BUILTIN_EXC_PARENTS,
    BUILTIN_EXC_IDS,
    BUILTIN_TYPE_IDS,
...
class Codegen:
    ...
    def generate(self) -> str:
        ...
        # Drain lambdas registered by `_gen_lambda` while generating the
        # method bodies above (and any lambdas nested inside those lambdas'
        # own bodies) — they were appended to `self.mod.funcs` too late for
        # the cursor loop above to see them.
        while funcs_emitted < len(self.mod.funcs):
    ...
    def _closure_factory_body(self, value) -> "Optional[list]":
        ...
        if isinstance(value, A.MethodCall):
            obj_type: str = A.expr_type(value.obj)
        ...
        if isinstance(expr, A.ListLit):
            ...
            if A.expr_type(expr.iter) == "list":
                if isinstance(expr.iter, A.Name):
                    ...
                    var_ty: str = getattr(expr.iter, "list_el_type", "int")
            if expr.targets:
            ...
            lt: str = A.expr_type(expr.left)
            rt: str = A.expr_type(expr.right)
            if "str" in (lt, rt):
            ...
            if expr.func in ("bytes", "bytearray"):
                if not expr.args:
                    ...
                    at: str = A.expr_type(expr.args[0]) if expr.args else "int"
                    if at == "str":
            ...
            kwlist: list = getattr(expr, "kwargs", [])
            ...
            if not (
                ...
                obj_t: str = A.expr_type(expr.obj)
            ...
            if isinstance(s, A.MultiAssign):
                ...
                if A.expr_type(inner) == "list":
                    if isinstance(inner, A.Name):
                        ...
                        el_ty: str = getattr(inner, "list_el_type", "int")
    ...
    def gen_stmt(self, stmt, info: FuncInfo) -> None:
        ...
        if isinstance(stmt, A.Assign):
            ...
            value_t: str = A.expr_type(stmt.value)
        ...
        if isinstance(stmt, A.TupleAssign):
            ...
            if len(stmt.values) == 1 and (
                ...
                # Sema guarantees plain-name targets in this form.
                ptr_slot = info.locals_[f"__tupunpack_{id(stmt)}"]
                rhs_t: str = A.expr_type(stmt.values[0])
            ...
            for i, target in enumerate(stmt.targets):
                ...
                if isinstance(target, A.Name):
                    ...
                    obj_t: str = A.expr_type(target.obj)
                    if obj_t == "dict":
        ...
        if isinstance(stmt, A.While):
            ...
            orelse_w: list = getattr(stmt, "orelse", [])
            if orelse_w:
        ...
        if isinstance(stmt, A.IndexAssign):
            ...
            if isinstance(stmt.target.index, A.Slice):
                ...
                return
            obj_t: str = A.expr_type(stmt.target.obj)
    ...
    def _gen_for(self, stmt: A.For, info: FuncInfo) -> None:
        ...
        if stmt.iter is not None:
            iter_t: str = A.expr_type(stmt.iter)
        ...
        orelse_f: list = getattr(stmt, "orelse", [])
    ...
    def gen_expr(self, expr: A.Expr, info: FuncInfo) -> None:
        ...
        if isinstance(expr, A.UnaryOp):
            operand_t: str = A.expr_type(expr.operand)
    ...
    def _gen_named_expr(self, e: A.NamedExpr, info: FuncInfo) -> None:
        ...
        value_t: str = A.expr_type(e.value)
    ...
    def _gen_tuple_lit(self, e: A.TupleLit, info: FuncInfo) -> None:
        ...
        for i, el in enumerate(e.elems):
            ...
            el_t: str = e.elem_types[i] if i < len(e.elem_types) else "int"
            if el_t == "float":
                self.emitf(f"movsd [rcx+{i * 8}], xmm0")
            else:
                self.emitf(f"mov [rcx+{i * 8}], rax")
    ...
    def _gen_subscript(self, e: A.Subscript, info: FuncInfo) -> None:
        if isinstance(e.index, A.Slice):
            obj_t_sl: str = A.expr_type(e.obj)
            if obj_t_sl == "list":
        ...
        if obj_t == "set":
            ...
            if e.method == "update":
                ...
                if arg_t in ("list", "tuple"):
                    ...
                    el_t: str = getattr(e.args[0], "list_el_type", None) or "any"
                    self.label(loop)
                    self.emitf(
    ...
    def _gen_compare(self, e: A.Compare, info: FuncInfo) -> None:
        ...
        if len(e.ops) == 1 and e.ops[0] in ("in", "not in"):
            ...
            if getattr(e, "dunder_contains_owner", None) is not None:
                ...
                return
            rt: str = A.expr_type(e.operands[1])
            if rt in ("list", "tuple"):
        ...
        # literals.
        lt0: str = A.expr_type(e.operands[0])
        rt0: str = A.expr_type(e.operands[1])
    ...
    def _gen_ffi_call(self, fn: stdlib.Func, args: list, info: FuncInfo) -> None:
        ...
        slot_offs: list[int] = []
        for i, (arg, want_raw) in enumerate(zip(args, fn.arg_types)):
            want: str = want_raw
            got = A.expr_type(arg)
            if want == "float":
    ...
    def _emit_print_value(self, expr, info: FuncInfo) -> None:
        ...
        t: str = A.expr_type(expr)
        spec: str = getattr(expr, "fmt_spec", "")
        conv: str = getattr(expr, "conv_flag", "")
    ...
    def emit_asmlib_runtime(self) -> None:
        ...
        Called by each hosted target's emit_print_impls when _net_* or _gui_*
        symbols appear in ffi_externs.  The default implementation is a
