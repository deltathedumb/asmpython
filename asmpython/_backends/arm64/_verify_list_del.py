"""Verify the valid-index freestanding ARM64 list deletion runtime path."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="asmpython-arm64-list-del-") as tmp:
        executable = Path(tmp) / "program"
        program_blob = compile_source_object(_DEL_SOURCE)
        requirements = required_external_symbols(program_blob)
        if requirements != _EXPECTED_REQUIREMENTS:
            raise SystemExit(
                "list-deletion probe lowered to an unexpected external-symbol set: "
        ...
        if completed.returncode != 0 or completed.stdout != _EXPECTED_STDOUT:
            ...
            raise SystemExit(
                "freestanding ARM64 list-deletion probe failed: "
