"""Verify the first freestanding ARM64 list-runtime slice."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="asmpython-arm64-list-core-") as tmp:
        executable = Path(tmp) / "program"
        program_blob = compile_source_object(_LIST_SOURCE)
        requirements = required_external_symbols(program_blob)
        if requirements != _EXPECTED_REQUIREMENTS:
            raise SystemExit(
                "list-core probe lowered to an unexpected external-symbol set: "
        ...
        if completed.returncode != 0 or completed.stdout != _EXPECTED_STDOUT:
            ...
            raise SystemExit(
                "freestanding ARM64 list-core probe failed: "
