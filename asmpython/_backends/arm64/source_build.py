...
def _normalize_ffi_return_conversions(module: Any) -> None:
    ...
    Shared IR lowering currently types those calls by their Python-visible
    ``ret_type`` (I64), while the actual C ABI callee returns a double in D0.
    AArch64 must therefore capture the call into an F64 temporary and convert
    that temporary with the already-supported ``fptosi`` operation. Keeping the
    conversion explicit in IR also lets register allocation see the real GP/FP
    transition instead of teaching call codegen about individual libm symbols.
    """
    for func in getattr(module, "funcs", ()):
        ...
        for block in getattr(func, "blocks", ()):
            ...
            for instr in getattr(block, "instrs", ()):
                target = instr.operands[0] if instr.op == "call" and instr.operands else None
                if (
                    instr.op == "call"
                    ...
                    rewritten.append(IRInstr("call", raw_result, list(instr.operands)))
                    rewritten.append(IRInstr("fptosi", original_result, [raw_result]))
                else:
