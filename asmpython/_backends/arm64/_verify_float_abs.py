"""Verify exact ARM64 float absolute-value runtime behavior."""
...
def main() -> int:
    print(int(fabs(-3.5)), int(abs(-2.75)), int(fabs(-0.0)))
    return 0
"""
_EXPECTED_STDOUT = "3 2 0\n"
_EXPECTED_REQUIREMENTS = frozenset({"_abi_int_to_base", "fabs", "printf"})
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="asmpython-arm64-fabs-") as tmp:
        executable = Path(tmp) / "program"
        program_blob = compile_source_object(_FLOAT_ABS_SOURCE)
        requirements = required_external_symbols(program_blob)
        if requirements != _EXPECTED_REQUIREMENTS:
            raise SystemExit(
                "float-abs probe lowered to an unexpected external-symbol set: "
        ...
        if completed.returncode != 0 or completed.stdout != _EXPECTED_STDOUT:
            ...
            raise SystemExit(
                "freestanding ARM64 float-abs probe failed: "
                f"returncode={completed.returncode}"
            )

        mode_name = "native AArch64" if toolchain.native else "qemu-aarch64"
        print("[ OK ] math.fabs and builtin abs(float) shared exact fabs")
        print("[ OK ] negative zero became positive zero before int conversion")
