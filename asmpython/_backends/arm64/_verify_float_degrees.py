"""Verify ARM64 math.degrees with the established binary64 factor."""
...
def main() -> int:
    print(int(degrees(0.0)), int(degrees(pi)), int(degrees(-1.5707963267948966)), int(degrees(1.0)))
    return 0
"""
_EXPECTED_STDOUT = "0 180 -90 57\n"
_EXPECTED_REQUIREMENTS = frozenset({"_abi_int_to_base", "_math_degrees", "printf"})
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="asmpython-arm64-degrees-") as tmp:
        executable = Path(tmp) / "program"
        program_blob = compile_source_object(_DEGREES_SOURCE)
        requirements = required_external_symbols(program_blob)
        if requirements != _EXPECTED_REQUIREMENTS:
            raise SystemExit(f"degrees probe external symbols: expected={sorted(_EXPECTED_REQUIREMENTS)}, actual={sorted(requirements)}")
        ...
        if completed.returncode != 0 or completed.stdout != _EXPECTED_STDOUT:
            ...
            raise SystemExit(f"freestanding ARM64 degrees probe failed: returncode={completed.returncode}")
        mode_name = "native AArch64" if toolchain.native else "qemu-aarch64"
        print("[ OK ] degrees used the established 180/pi binary64 factor")
