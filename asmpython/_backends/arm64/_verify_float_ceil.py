...
_EXPECTED_REQUIREMENTS = frozenset(
    {"_abi_int_to_base", "_math_isinf", "_math_isnan", "ceil", "copysign", "printf"}
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="asmpython-arm64-ceil-") as tmp:
        ...
        if actual != _EXPECTED_REQUIREMENTS:
            raise SystemExit(
                f"ceil probe external symbols: expected={sorted(_EXPECTED_REQUIREMENTS)}, "
        ...
        if completed.returncode != 0 or completed.stdout != _EXPECTED_STDOUT:
            raise SystemExit(
                f"ARM64 ceil probe failed: returncode={completed.returncode}, "
