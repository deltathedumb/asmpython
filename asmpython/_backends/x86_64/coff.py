"""
COFF AMD64 relocatable object file emitter.

Produces a .o / .obj file that gcc (MinGW), clang, and MSVC link.exe can link:
    gcc -o program main.o your_output.o        (MinGW)
    link.exe /out:program.exe main.obj out.obj  (MSVC)
...
DATA_CHARS  = (IMAGE_SCN_CNT_INITIALIZED_DATA | IMAGE_SCN_ALIGN_8BYTES
...
IMAGE_REL_AMD64_REL32     = 4    # PC-relative 32-bit (for CALLs and LEAs)
IMAGE_REL_AMD64_ADDR32NB  = 3    # 32-bit address without image base (for data refs)
...
def _reloc_entry(offset: int, sym_idx: int, rtype: int = IMAGE_REL_AMD64_REL32) -> bytes:
...
def _build_strtab(long_names: "list[str]") -> "tuple[bytes, dict[str, int]]":
    data = bytearray(4)             # 4-byte size placeholder
    offs: dict[str, int] = {}
...
def build_coff(
    ...
    """
    Build a COFF AMD64 relocatable object file.
    ...
    for fc in func_codes:
        ...
        for patch_off, sym, _rtype in fc.relocs:
            # COFF uses REL32 for all PC-relative refs (calls + data LEAs)
    ...
    # ── File layout ───────────────────────────────────────────────────────────
    # [COFF header] [section headers...] [.text] [.data?] [.rdata?] [relocs] [symtab] [strtab]
