"""Parser for the relocatable COFF object files coff.py emits (and that
NASM/MSVC produce too, for the symbol/section/relocation shapes coff.py
actually uses -- this is not a general COFF reader). Used by pe_linker.py
...
def parse_coff(data: bytes) -> CoffObject:
    ...
    # Symbol table + string table (string table immediately follows symtab).
    ...
    # Aux symbol records (FILE/SECTION entries NASM emits) consume extra
    # 18-byte slots right after the symbol they belong to, with no name/
    # value/section fields of their own -- skip num_aux of them or every
    # later symbol index is off by however many aux slots came before it.
    symbols: list[CoffSymbol] = []
    i = 0
    while i < num_syms:
        ...
        i += 1 + naux
        for _ in range(naux):
            symbols.append(CoffSymbol("", 0, 0, 0, 0))  # placeholder so indices still align
