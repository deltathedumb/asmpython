...
``asmpython.extras`` is a deprecated alias of ``asmpython.annotations``, kept
as a compatibility namespace.  Every decorator stores immutable-ish,
serializable metadata on ``__asmpython_metadata__`` so CPython tooling and the
native compiler can consume the same declarations.
"""
