...
```python
from apc.link import Toolchain, LinkRequest, LinkError, find_tool, run, register
...
```bash
apc build prog.py --toolchain bare-metal
apc toolchains                    # lists it
```
...
something has to define it. `apc.link.runtime` supplies a small C file for the
...
The IR's `main` is emitted under `ENTRY_SYMBOL` (`apc_main`), because it is
not C's `main` — it returns i64 where C requires int, and would collide with
the runtime's entry point. That constant lives in `apc.backend.base` so the
...
- [ ] the produced program runs, and its output matches `apc run` on the same
      source
