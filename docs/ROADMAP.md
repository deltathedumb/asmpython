...
- **Memory management** — heap objects are never freed. `free` appears exactly twice in the whole runtime, both inside `_runtime_dict_grow` releasing the old buffers after a rehash; every other allocation leaks for the life of the process. A *precise* collector is an object-model change, not a bolt-on: there is no object header, so nothing can say what a heap word is or how big an object is — which is exactly why `_lower_read_any_tag` has to guess (`> 0x10000`, 8-byte aligned, then a magic probe). The prerequisite is a uniform header, and it pays for itself independently by making that guess exact.
