...
- **Memory management** — ~~heap objects are never freed~~ **done on Windows, inert on Linux**: a stop-the-world mark-sweep over a registry of tracked objects, each carrying a 16-byte `[meta][next]` header (`meta` = size | mark | kind<<32). `gc.collect()` returns the number of objects actually reclaimed, and collection also runs automatically every `gc.get_threshold()[0]` object allocations (default 700, CPython's gen-0 number) once armed by `gc.enable()`. The trigger lives inside `_runtime_objalloc` and fires **before** the allocation — after it, the fresh pointer would live only in a caller-saved register with nothing rooting it. Roots: the machine stack, every writable section of the program image, and any exact roots on the shadow stack. Neither root set needs cooperation from the backend, which matters because the default x86-64 backend emits no entry prologue and so never calls `_runtime_gc_init` — Win64 reads the stack base from the TEB at `gs:8` and finds its globals by walking its own PE headers from the PEB at `gs:0x60`, checking both the `MZ` and `PE` signatures so that a misparse yields an EMPTY range rather than a plausible wrong one. **The collector refuses to sweep unless it can see both root sets.** Missing either is not a degraded collection but a wrong one: skip the stack and every object held only by a local is freed; skip the globals and every object held only by a module-level name is freed. So a missing root set means `collect()` returns 0 and frees nothing — which is exactly the state Linux is in until it grows the equivalent hooks. Element BUFFERS stay unregistered on purpose: a buffer has no identity, dies with its owner, and `realloc` moves it, which would invalidate a registry link on every `append`; the tracer reaches a list's elements through the owner's `kind` instead. Refcounting for deterministic `del`/`__del__` is specified and refused, not implemented.
