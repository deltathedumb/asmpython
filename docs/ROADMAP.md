...
- **Memory management** — ~~heap objects are never freed~~ *partly done*: `--gc=on` is a stop-the-world mark-sweep over a registry of tracked objects, each carrying a 16-byte `[meta][next]` header (`meta` = size | mark | kind<<32). Roots are the machine stack, the module-globals area, and any exact roots on the shadow stack; registry membership is the "is this word really an object" test, which replaces the old `> 0x10000`-and-probe guess for traced words. Element BUFFERS stay unregistered on purpose — a buffer has no identity, dies with its owner, and `realloc` moves it, which would invalidate a registry link on every `append`; the tracer reaches a list's elements through the owner's `kind` instead. Remaining: (1) nothing triggers collection automatically, so only an explicit `gc.collect()` reclaims; (2) the mode is honoured only by the legacy targets that emit the entry prologue carrying it; (3) Linux has no stack-base hook (`__libc_stack_end`) yet, so the stack scan is skipped there — and a collector that frees live data is worse than no collector, which is why the default stays `off`; (4) refcounting for deterministic `del`/`__del__` is specified and refused, not implemented.
