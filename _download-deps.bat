...
REM w64devkit - minimal, self-contained GCC toolchain for Windows.
set "GCC_TAG=1.23.0"
set "GCC_FILE=w64devkit-1.23.0.zip"
set "GCC_URL=https://github.com/skeeto/w64devkit/releases/download/v%GCC_TAG%/%GCC_FILE%"

set "PY_VER=3.12.7"
set "PY_URL=https://www.python.org/ftp/python/%PY_VER%/python-%PY_VER%-embed-amd64.zip"

REM Temp zip paths — set at top-level so they resolve correctly inside call'd subroutines.
set "GCC_ZIP=%TEMP%\asmpython_gcc.zip"
set "NASM_ZIP=%TEMP%\asmpython_nasm.zip"
set "PY_ZIP=%TEMP%\asmpython_python.zip"
...
if "%DO_GCC%"=="1"    echo   gcc:    %DEST%\w64devkit\bin\gcc.exe
...
curl.exe -fL --progress-bar -o "%NASM_ZIP%" "%NASM_URL%"
if errorlevel 1 exit /b 1
if exist "%DEST%\nasm" rmdir /s /q "%DEST%\nasm"
echo Extracting...
tar.exe -xf "%NASM_ZIP%" -C "%DEST%"
if errorlevel 1 exit /b 1
if exist "%DEST%\nasm-%NASM_VER%" move "%DEST%\nasm-%NASM_VER%" "%DEST%\nasm" >nul
del "%NASM_ZIP%"
if not exist "%DEST%\nasm\nasm.exe" (
...
:get_gcc
echo === GCC ^(w64devkit %GCC_TAG%^) ===
echo Downloading %GCC_URL%
curl.exe -fL --progress-bar -o "%GCC_ZIP%" "%GCC_URL%"
if errorlevel 1 exit /b 1
if exist "%DEST%\w64devkit" rmdir /s /q "%DEST%\w64devkit"
echo Extracting...
tar.exe -xf "%GCC_ZIP%" -C "%DEST%"
if errorlevel 1 exit /b 1
del "%GCC_ZIP%"
if not exist "%DEST%\w64devkit\bin\gcc.exe" (
    echo Error: gcc.exe not found after extraction. Archive layout may have changed.
    exit /b 1
)
echo OK: %DEST%\w64devkit\bin\gcc.exe
echo.
exit /b 0
...
curl.exe -fL --progress-bar -o "%PY_ZIP%" "%PY_URL%"
if errorlevel 1 exit /b 1
if exist "%DEST%\python" rmdir /s /q "%DEST%\python"
mkdir "%DEST%\python"
echo Extracting...
tar.exe -xf "%PY_ZIP%" -C "%DEST%\python"
if errorlevel 1 exit /b 1
del "%PY_ZIP%"
...
echo     --gcc         w64devkit GCC %GCC_TAG% (self-contained Win32 toolchain) (~60 MB)
