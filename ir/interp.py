...
class Frame:
    func: Func
    regs: dict[int, int | float] = field(default_factory=dict)
    frame_base: int = 0
...
class Interpreter:
    ...
    def _exec(self, fr: Frame, ins: Instr):
        op, ty = ins.op, ins.ty
        R = fr.regs
        ...
        if op is Op.FUNC_ADDR:
            return put(_FUNC_TAG | self.module.funcs.index(self.module.func(ins.sym)))
        ...
        if op is Op.BITCAST:
            return put(_bitcast(a(0), fr.func.reg_type(ins.args[0]), ty))
        ...
        if op is Op.CALL:
            callee = self.module.func(ins.sym)
            if callee is None:
        ...
        if op is Op.CALL_PTR:
            ...
            return put(self._call(self.module.funcs[idx],
