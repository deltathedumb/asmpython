...
from .core import Block, Func, Global, Instr, Module
from .ops import Op
...
def print_func(f: Func) -> str:
    params = ", ".join(f"%{p}: {f.regs.get(p, T.VOID)}" for p in f.params)
    ...
    for b in f.blocks:
        ...
        for ins in b.instrs:
            lines.append(_INDENT + print_instr(ins))
    lines.append("}")
    return "\n".join(lines)
...
def parse_module(text: str) -> Module:
    ...
    for n, raw in enumerate(text.splitlines(), 1):
        ...
        if f:
            ...
            fn = Func(name, T.parse(ret), exported=bool(exported),
        ...
        blk.instrs.append(_parse_instr(n, raw, line, fn))
...
def _parse_instr(n: int, raw: str, line: str, fn: Func) -> Instr:
    dst = None
    ...
    if dst is not None and dst not in fn.regs:
        fn.regs[dst] = T.I1 if ops.spec(op).result == "i1" else ty
    return ins
